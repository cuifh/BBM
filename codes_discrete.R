...
plot(kapprox,col="red",xlim=s,main = "")
...
for (i in 1:M){
  par(new=T)
  plot(list_r[[i]],col = alpha("pink",0.3),xlim=s,main="")
}



#########################################BEGIN###################
lines(c(-1,1,3,5,7),c(0,0.2,0.7,1,1), type = "s",col="blue",xlim=s)
par(new=T)
plot(kapprox,col="red",xlim = s,main="Discrete, sample size=500, bootstrap size=500")
##############END#################################################
